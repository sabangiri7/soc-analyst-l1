...
class DesignDetectionDashboard(BaseWazuhTool):
    ...
    def run(self, ctx: ToolContext, **params: Any) -> Any:
        ...
        panel_query_map = {p["slug"]: p["query"] for p in panels}

        # 2) verify each panel's query matches data (evidence)
        verified: list[dict[str, Any]] = []
        degraded: list[str] = []
        for p in panels:
            check = verify_opensearch_query(ctx.indexer, _INDEX, search_body(p["query"], size=0))
            if not check.get("valid"):
                degraded.append(p["slug"])
                check = {"valid": False, "error": check.get("error")}
            verified.append({"slug": p["slug"], "title": p["title"], "matched": check.get("matched", 0),
        ...
        x = 0
        for p in panels:
            vis_state = _vis_state(p["title"], p["vis_type"], p["aggs"])
            visualizations.append({
                "slug": p["slug"],
                "title": p["title"],
                "vis_type": p["vis_type"],
                "vis_state": vis_state,
                "search_source": _search_source(index_pattern, p["aggs"]),
            })
            grid.append({"id": f"vis-{p['slug']}", "x": x % 2 * 24, "y": (x // 2) * 15,
            ...
            for v in visualizations:
                resp = dashboards_request(
                    ...
                    body={
                        ...
                        "references": [{"id": p["index_pattern"],
                                        "name": "kibanaSavedObjectMeta.searchSourceJSON.index",
                                        "type": "index-pattern"}] if p["index_pattern"] else [],
                    },
                )
